...
//////////////////////////
// Color Logic (Aggressive vs Recommended)
//////////////////////////
function applyAggressiveColor(){
  const pairs = [
    ["rec_tex","agg_tex"],
  ...
  pairs.forEach(([recId, aggId])=>{
    const rec = document.getElementById(recId);
    const agg = document.getElementById(aggId);
    if(!rec || !agg) return;
    if(agg.classList.contains("highlight-warning")) return;
    const rv = (rec.innerText||"").trim();
    const av = (agg.innerText||"").trim();
    if(rv !== av){
      agg.classList.add("diff");
    }else{
      agg.classList.remove("diff");
